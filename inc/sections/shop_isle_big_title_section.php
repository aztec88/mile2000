...
if ( ( isset( $shop_isle_big_title_hide ) && $shop_isle_big_title_hide != 1 ) || is_customize_preview() ) {
	...
	if ( ! empty( $shop_isle_homepage_slider_shortcode ) ) {
		...
		if ( ! empty( $shop_isle_big_title_image ) ) {
			...
			echo '<h1 class="hs-title-size-4 font-alt mb-30">  '.__( 'Welcome', 'shop-isle' ).'</h1>';
			echo'<div class="hs-title-size-1 font-alt mb-40">'.__( 'Online-Shop', 'shop-isle' ).'</div>';
			echo '<a href="'.get_permalink(icl_object_id(5)).'" class="section-scroll btn btn-border-w btn-round">'.__( 'Shop now', 'shop-isle' ).'</a>';
